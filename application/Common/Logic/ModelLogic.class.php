...
class ModelLogic extends BaseLogic {
    ...
    public function deleteModel($model_data) {
        ...
        $model->startTrans();
		//'siteid' => get_siteid(),
        if ($model->where(array('id' => $model_data['id']))->delete() !== false &&
            $model_field->where(array('modelid' => $model_data['id']))->delete() !== false &&
//            $this->deleteModelMenu($model_data['tablename']) !== false &&
    ...
    public function addModel($data) {
        ...
        if ($modelid !== false) {
            $data['id'] = $modelid;
			//菜单添加：$this->addModelMenu($data) && 
            if ($this->execModelCreateSql($data['tablename'], $modelid, $data['siteid'])) {
                $model->commit();
                return true;
            } else {
                $model->rollback();
